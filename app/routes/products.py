...
# ═══════════════════════════════════════════════════════════════
# ⚠️  ROUTE ORDER IS CRITICAL IN FASTAPI
#
#  Static/prefix routes (/admin/..., /variants/..., /images/...)
#  MUST be registered BEFORE wildcard routes (/{product_id}).
#
#  If /{product_id} is registered first, FastAPI matches
#  "admin" as a product_id and returns 404 — breaking ALL
#  admin endpoints, bulk upload, and delete counts.
# ═══════════════════════════════════════════════════════════════
...
def list_products(
    ...
    # BUG FIX: isnot(True) passes NULL rows through in PostgreSQL.
    # == False correctly excludes both NULL and True.
...
# ─────────────────────────────────────────────
# ADMIN: LIST PRODUCTS  — static /admin/list
# ─────────────────────────────────────────────
...
def admin_list_products(
    ...
    if not include_deleted:
        # BUG FIX: == False excludes NULL rows; isnot(True) does not
...
def create_product(payload: dict, db: Session = Depends(get_db), admin=Depends(require_admin)):
    ...
    product.is_deleted = False  # BUG FIX: explicitly set to prevent NULL
    db.add(product)
    db.flush()
...
# ─────────────────────────────────────────────
# ADMIN: UPDATE PRODUCT  — static /admin/{id}
# ─────────────────────────────────────────────
...
# ─────────────────────────────────────────────
# ADMIN: ANALYTICS  — static /admin/{id}/analytics
# ─────────────────────────────────────────────
...
# ─────────────────────────────────────────────
# ADMIN: BULK OPERATIONS  — static /admin/bulk*
# ─────────────────────────────────────────────
...
# ─────────────────────────────────────────────
# ADMIN: BULK UPLOAD  — static /admin/bulk-upload
# BUG FIX: Was at bottom of file, below /{product_id},
# so FastAPI matched "admin" as product_id → 404.
# ─────────────────────────────────────────────
    ...
    for idx, row in enumerate(rows, 1):
        try:
            ...
            product = Product(
                ...
                status="active",
                is_deleted=False,  # BUG FIX: explicitly set to prevent NULL
            )
            db.add(product)
            db.flush()
...
# ─────────────────────────────────────────────
# ADMIN: IMPORT / EXPORT  — static /admin/import-* and /admin/export
# ─────────────────────────────────────────────
...
async def import_validate(file: UploadFile = File(...), db: Session = Depends(get_db)):
    ...
    return {"total_rows": len(rows), "errors": errors, "warnings": warnings, "valid": len(errors) == 0}
...
# ─────────────────────────────────────────────
# VARIANT ROUTES  — static /variants/...
...
# ⚠️  Wildcard /{product_id} — MUST be LAST among GET routes
# ─────────────────────────────────────────────
...
# ─────────────────────────────────────────────
# ADMIN: PRODUCT LIFECYCLE  — wildcard /{product_id}
# These must come AFTER all /admin/*, /variants/*, /images/* routes
# ─────────────────────────────────────────────
